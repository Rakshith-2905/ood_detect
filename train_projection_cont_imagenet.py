...
def get_save_dir(args):
    base_dir = f"logs/classifier/imagenet/contrastive_loss/{args.resnet_model}_{args.dataset}_{args.domain}"
...
def train_one_epoch(train_loader, resnet_model, projector, text_encodings, criterion, optimizer, device, epoch, label_mapping=None):
    ...
    for resnet_logits, resnet_embeddings, gt_labels in pbar:

        resnet_logits, resnet_embeddings, gt_labels = resnet_logits.to(device), resnet_embeddings.to(device), gt_labels.to(device)
...
def main(args):
    ...
    train_loader,val_loader = get_data_from_saved_files(f'imagenet_feats/{args.resnet_model}_imagenet_features.pkl', args.batch_size, train_shuffle=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--resnet_model', type=str, choices=['resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152'], default='resnet50', help='Type of ResNet model to use')
    parser.add_argument('--checkpoint_path', type=str, required=False,help='Path to checkpoint to resume training from')
