...
def get_save_dir(args):
    ...
    save_dir = os.path.join(args.save_dir, args.dataset_name, 'merging_priors', projector_name)
...
def validate(data_loader, limber, 
    ...
    for images_batch, labels, images_clip_batch in pbar:
        ...
        # T100 is the logits scale from CLIP (batch level classification of images and text)
    ...
    return total_loss, total_limber_acc, total_classifier_acc, total_image_loss, total_text_loss
...
def main(args):
    ...
    limber.load_checkpoint(args.checkpoint_path)
    limber = fabric.to_device(limber)
    
    ########################### Load the dataset ############################

    # Create the data loader and wrap them with Fabric
    _, val_dataset, test_dataset, _, class_names = get_dataset(args.dataset_name, train_transform, test_transform, 
    ...
    start_epoch = 0
    val_loss, val_limber_acc, val_classifier_acc, val_loss_img, val_loss_txt = validate(val_loader, limber, class_prompts,text_encodings, epoch=1)
    fabric.print(f"Val Loss: {val_loss:.4f}, Val classifier Accuracy: {val_classifier_acc:.4f} Val limber Accuracy: {val_limber_acc:.4f}")

    test_loss, test_limber_acc, test_classifier_acc, test_loss_img, test_loss_txt = validate(test_loader, limber, class_prompts,text_encodings, epoch=1)
    fabric.print(f"Test Loss: {test_loss:.4f}, Test limber Accuracy: {test_limber_acc:.4f}")

    losses_dict = {"val_loss": val_loss, "val_loss_img": val_loss_img, "val_loss_txt": val_loss_txt, "val_limber_acc": val_limber_acc, "val_classifier_acc": val_classifier_acc,
                   "test_loss": test_loss, "test_loss_img": test_loss_img, "test_loss_txt": test_loss_txt, "test_limber_acc": test_limber_acc, "test_classifier_acc": test_classifier_acc}
    fabric.log_dict(losses_dict, step=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint_path', type=str, help='Path to checkpoint to load different components')
    ...
    parser.add_argument('--use_pretrained', action='store_true', help='Whether to use imagenet pretrained weights or not')
    ...
    parser.add_argument('--classifier_dim', type=int, default=None, help='Dimension of the classifier embeddings')
...
python merging_priors_test.py \
    ...
    --seed 42 \
    --img_size 224 \
    --img_projection --cls_txt_prompts  \
    --classifier_name 'resnet50_adv_l2_0.1' \
