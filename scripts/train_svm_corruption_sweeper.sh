...
classifier_name='SimpleCNN' # (resnet18, SimpleCNN)
num_epochs=10
data_dir='./data/'
classifier_checkpoint_path="logs/cifar10/${classifier_name}/classifier/checkpoint_29.pth"
        ...
        for a in "${teacher_temps[@]}"; do
            for b in "${weight_img_loss[@]}"; do
                for c in "${weight_txt_loss[@]}"; do
                    ...
                    step1_checkpoint_path="logs/cifar10/${classifier_name}/${model_type}/_clsEpoch_29_bs_128_lr_0.1_teT_${a}_sT_1.0_imgweight_${b}_txtweight_${c}_is_mlp_False/step_1/best_projector_weights.pth"
                    python train_SVM.py \
                            --data_dir "$data_dir"  \
                            --domain_name "$corr"    \
                            --severity 3 \
                            ...
                            --weight_img_loss $b  \
                            --weight_txt_loss $c \
                            --svm_features classifier_features
                done
            done
        done
    done
