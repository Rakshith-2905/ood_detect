...
gt_concept_set='clip-dissect/NICOpp_att_concepts.json'
concept_set='clip-dissect/concepts/niccopp_concepts_att.txt'
    ...
    for a in "${teacher_temps[@]}"; do
        for b in "${weight_img_loss[@]}"; do
            for c in "${weight_txt_loss[@]}"; do
                checkpoint_path="logs/${dataset_name}/shift_detection/att_${atts}/${model_type}/_bs_256_lr_0.1_teT_${a}_sT_1.0_imgweight_${b}_txtweight_${c}_is_mlp_False/best_projector_weights.pth"
