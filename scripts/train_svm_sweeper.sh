...
dataset_name='cifar10-limited' # (CelebA, Waterbirds, cifar10, cifar10-limited)
    ...
    for a in "${teacher_temps[@]}"; do
        for b in "${weight_img_loss[@]}"; do
            for c in "${weight_txt_loss[@]}"; do
                ...
                step1_checkpoint_path="logs/${dataset_name}/${classifier_name}/${model_type}/_clsEpoch_29_bs_128_lr_0.1_teT_${a}_sT_1.0_imgweight_${b}_txtweight_${c}_is_mlp_False/step_1/best_projector_weights.pth"
                python train_SVM.py \
                        --data_dir "$data_dir"  \
                        --domain_name 'real'    \
